...
def get_arguments():
    ...
    parser.add_argument(
        '-o', '--output_dir', type=Path, default=Path("/data/alex/indo_pacific"),
...
NETWORK_STATION_CODES = np.loadtxt('network_station_codes.txt', dtype=str)
...
def main():
    ...
    domain = CircularDomain(latitude=5, longitude=117, minradius=0, maxradius=40)

    mdl = MassDownloader(providers=["IRIS", 'GEONET', 'SCEDC', 'USGS', 'GFZ', 'NCEDC', 'RESIF', 'INGV', 'ETH', 'LMU', 'KOERI', 'INGV', 'EMSC', 'BGR', 'ICGC', 'IESDMC'])
    for network, station in NETWORK_STATION_CODES:
        print(network, station, flush=True)
        try:
            restrictions = Restrictions(
                starttime=obspy.UTCDateTime(2024, 9, 1),
                endtime=obspy.UTCDateTime(2024, 9, 20),
                network=network,
                station=station,
                ...
                minimum_length=0.0,
                minimum_interstation_distance_in_m=1000000
            )
            mdl.download(
                domain, restrictions,
            ...
            inv = obspy.read_inventory(str(output_dir / "stations.xml" / f"{network}.{station}.xml"))
